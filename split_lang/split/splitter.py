...
class LangSplitter:
    ...
    def _smart_merge(
        ...
        lang_text_list = self._merge_substrings(lang_text_list)
        if self.debug:
            logger.debug("---------after _merge_middle_substr_to_two_side:")
            for substr in lang_text_list:
                logger.debug(substr)
        ...
        lang_text_list = self._merge_substrings(lang_text_list)
        if self.debug:
            logger.debug("---------after _merge_side_substr_to_near:")
            for substr in lang_text_list:
                logger.debug(substr)
        lang_text_list = self._get_languages(
            lang_text_list=lang_text_list,
    ...
    def _is_cur_short_and_near_long(self, cur: SubString, near: SubString):
        ...
        is_near_contains_no_kana = not contains_ja_kana(near.text)
        is_both_contains_no_kana = is_cur_contains_no_kana and is_near_contains_no_kana
        # NOTE: e.g. 日本人, 今晚, 国外移民
        is_near_str_is_long_zh = (
            near.length > cur.length and near.length >= 6 and near.lang == "zh"
        )
        is_cur_short_and_near_is_zh_and_long = (
            is_near_str_is_long_zh and is_both_contains_no_kana
        )
        cur_lang_is_possible_the_same_as_near = (
            near.lang in possible_detection_list(cur.text) and is_both_contains_no_kana
        )
        return (
            is_cur_short_and_near_is_zh_and_long
